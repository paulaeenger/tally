...
export function StatCard({ label, value, change, trend, footnote, className }: StatCardProps) {
  ...
  return (
    <div className={cn('card p-5 sm:p-6', className)}>
      ...
      <p className="mt-2 font-display text-display-md font-normal tabular text-foreground">
        {value}
      </p>
      {(change || footnote) && (
        <div className="mt-3 flex items-center gap-2 text-xs">
          ...
          {change?.label && <span className="text-faint">{change.label}</span>}
          {footnote && !change && <span className="text-faint">{footnote}</span>}
        </div>
      )}
    </div>
  );
